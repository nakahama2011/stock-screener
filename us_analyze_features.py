...
使い方:
    python3 us_analyze_features.py
    python3 us_analyze_features.py --sample  # 50銘柄で高速テスト

出力:
    us_results/us_feature_analysis_report.html
...
# 同じディレクトリの us_backtester.py を読み込む
...
# 曜日
WEEKDAY_NAMES = {0: "月曜(Mon)", 1: "火曜(Tue)", 2: "水曜(Wed)", 3: "木曜(Thu)", 4: "金曜(Fri)"}


def run_analysis(use_sample: bool = False):
    """メイン分析処理を実行する。"""
    print("=" * 60)
    print("📊 米国株 特徴量自動分析システム")
    if use_sample:
    ...
    results["combos"] = combo_results[:30]
    ...
    report_path = os.path.join(OUTPUT_DIR, "us_feature_analysis_report.html")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(html)

    # TOP30組み合わせをJSONで保存する
    import json
    combo_json = {
    ...
    combo_json_path = os.path.join(OUTPUT_DIR, "us_top_combos.json")
...
<p class="note">※ 各特徴量の範囲別に勝率を集計し、全体勝率との差を表示しています。</p>
{bin_tables}
...
<p class="note" style="text-align:center; margin-top:2rem">
    ...
    基準: SMA5&gt;SMA20&gt;SMA60, 出来高≥{DEFAULT_MIN_VOLUME:,}株 ・ 対象: S&amp;P 500
</p>
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="米国株 特徴量自動分析システム")
