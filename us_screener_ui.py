...
# -- レポート用HTTPサーバー（ポート8503）をバックグラウンドで起動する ----------
import socket, threading, http.server, functools

def _start_report_server():
    """us_results/ ディレクトリを配信する軽量HTTPサーバーを起動する。"""
    _results_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "us_results")
    if not os.path.isdir(_results_dir):
        return
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as _s:
        if _s.connect_ex(("localhost", 8503)) == 0:
            return
    handler = functools.partial(http.server.SimpleHTTPRequestHandler, directory=_results_dir)
    server = http.server.HTTPServer(("0.0.0.0", 8503), handler)
...
# us_backtester.py と同じディレクトリから import
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from us_backtester import (
    fetch_sp500_tickers,
    _fallback_tickers,
...
<div class="app-header">
  <h1>🇺🇸 米国株 日付指定スクリーニング <span class="badge-us">US STOCKS</span></h1>
  <p>指定した日付の終値時点で条件を満たす米国株を抽出し、翌日騰落率を一覧表示します</p>
</div>
""", unsafe_allow_html=True)

# 特徴量分析レポートへのリンク
_report_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "us_results", "us_feature_analysis_report.html")
if os.path.exists(_report_path):
    st.markdown(
        '<div style="text-align:right;margin-bottom:1rem">'
        '<span style="color:#94a3b8;font-size:0.9rem">'
        '👈 左サイドバーの「🇺🇸 米国株特徴量分析」ページでレポート全画面表示'
        '</span></div>',
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    while len(quick_dates) < 5:
        ...
        d -= timedelta(days=1)
    quick_labels = [
        f"{d.strftime('%m/%d')}（{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][d.weekday()]}）"
...
def run_single_day_screen(
    ...
    date_labels: dict = {}
    for _sym, (_name, _df) in all_data.items():
    ...
    if "_volume_ratio_raw" in result_df.columns and "出来高" in result_df.columns:
        # 終値は_screen_resultから取得
        close_prices = result_df["_screen_result"].apply(lambda x: x.get("close", 0) if isinstance(x, dict) else 0) if "_screen_result" in result_df.columns else pd.Series(0, index=result_df.index)
        ...
        result_df["推定売買可能額"] = vol_20ma * close_prices * 0.01
...
if run_button:
    ...
    st.session_state["us_result_df"] = result_df
    st.session_state["us_result_err"] = err_msg
    st.session_state["us_result_date"] = as_of_str
    st.session_state["us_result_hit_thr"] = hit_threshold
    st.session_state["us_date_labels"] = date_labels


# =====================================================
# 結果表示
# =====================================================
if "us_result_df" in st.session_state:
    result_df = st.session_state["us_result_df"]
    err_msg = st.session_state.get("us_result_err", "")
    as_of_str = st.session_state.get("us_result_date", "")
    saved_hit_thr = st.session_state.get("us_result_hit_thr", hit_threshold)
    ...
    if err_msg:
        ...
        if search_input:
            mask = (
                display_df["ティッカー"].astype(str).str.contains(search_input, na=False, case=False) |
                display_df["銘柄名"].str.contains(search_input, na=False, case=False)
            )
        ...
        if sort_col == "AI予測（降順）" and "AI予測(%)" in display_df.columns:
            ...
            display_df = display_df.sort_values("ティッカー")
        ...
        def _fmt_val(col, val):
            ...
            if col == "推定売買可能額":
                if v >= 1e6:
                    return f"${v/1e6:.1f}M", "color:#6366f1;font-weight:bold"
                elif v >= 1e3:
                    return f"${v/1e3:.0f}K", "color:#6366f1"
                else:
        ...
        for _, row in display_df.iterrows():
            symbol = str(row.get("ティッカー", ""))
            name = str(row.get("銘柄名", ""))
            tv_url = f"{tv_base}{symbol}"

            code_cell = (
                f'<td class="code-cell">'
                f'<a class="code-link" href="{tv_url}" target="_blank">{symbol}</a>'
                f'</td>'
            )
  ...
  ths.forEach(function(th, i){{
    ...
    th.addEventListener('click', function(){{
      ...
      rows.sort(function(a, b){{
        ...
        let cmp = bothNum ? (aNum - bNum) : aRaw.localeCompare(bRaw,'en');
    ...
    hideT = setTimeout(function(){{ tip.style.display='none'; }}, 200);
        ...
        with col_dl1:
            ...
            st.download_button(
                label="⬇️ CSV ダウンロード",
                data=csv_data,
                file_name=f"us_screening_{as_of_str.replace('-','')}.csv",
